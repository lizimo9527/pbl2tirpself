...
const mapConfig = {
  // 腾讯地图开发者密钥（需要申请）
  key: "WAOBZ-VZZKB-BY5UJ-NTNHU-5YXJQ-JEBWH",
  // 腾讯地图API密钥
  // 地图默认配置
  defaultCenter: {
    longitude: 116.3974,
    ...
    return new Promise((resolve, reject) => {
      common_vendor.index.request({
        ...
        data: {
          ...
          output: "json",
          mode
          // driving:驾车, walking:步行, bicycling:骑行
        },
        success: (res) => {
          if (res.data.status === 0) {
            resolve(res.data.result);
...
//# sourceMappingURL=../../.sourcemap/mp-weixin/config/map.config.js.map
