...
class GuideService {
  ...
  initTemplateGuides() {
    ...
    if (newTemplates.length > 0) {
      ...
      common_vendor.index.setStorageSync(this.storageKey, JSON.stringify(updatedGuides));
    }
  }
  // 获取模板攻略数据
  getTemplateGuides() {
...
- 晚上：结束长城之旅`,
  ...
  getGuides() {
    try {
      const guides = common_vendor.index.getStorageSync(this.storageKey);
      return guides ? JSON.parse(guides) : [];
    } catch (error) {
      common_vendor.index.__f__("error", "at services/guideService.js:226", "获取攻略列表失败:", error);
      return [];
    }
  }
  // 获取所有攻略（兼容旧版本）
  ...
  saveGuide(guideData) {
    try {
      ...
      common_vendor.index.setStorageSync(this.storageKey, JSON.stringify(guides));
      return true;
    } catch (error) {
      common_vendor.index.__f__("error", "at services/guideService.js:274", "保存攻略失败:", error);
      return false;
    }
  }
  // 删除攻略
  deleteGuide(id) {
    try {
      ...
      common_vendor.index.setStorageSync(this.storageKey, JSON.stringify(filteredGuides));
      return true;
    } catch (error) {
      common_vendor.index.__f__("error", "at services/guideService.js:287", "删除攻略失败:", error);
      return false;
    }
  }
  // 获取用户创建的攻略
  ...
  searchGuides(keyword) {
    ...
    return guides.filter(
      (guide) => guide.title.toLowerCase().includes(lowerKeyword) || guide.destination.toLowerCase().includes(lowerKeyword) || guide.tags && guide.tags.some((tag) => tag.toLowerCase().includes(lowerKeyword))
    );
  }
}
const guideService = new GuideService();
exports.guideService = guideService;
//# sourceMappingURL=../../.sourcemap/mp-weixin/services/guideService.js.map
